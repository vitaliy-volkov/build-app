...
import { X, TrendingUp, TrendingDown, Target, Calendar, DollarSign, FileText, 
         Building2, User, AlertTriangle, Brain, CheckCircle, Clock } from 'lucide-react';
import { 
  Transaction, 
  TransactionStatus, 
  OperationType, 
...
type PaymentTab = 'income' | 'expense' | 'planned';
...
export const PaymentManagementModal: React.FC<PaymentManagementModalProps> = ({
  ...
  // Обновление формы при смене таба
  useEffect(() => {
    setForm(prev => ({
      ...prev,
      type: activeTab === 'income' ? 'income' : 'expense'
  ...
  const validateWithAI = async () => {
    ...
    try {
      ...
      // Для планируемых платежей генерируем прогноз
  ...
  // Генерация прогноза для планируемых платежей
  const generateForecast = async () => {
    // TODO: Реализовать прогноз кэшфлоу
    setForecast([]);
  };
  ...
  const handleSave = () => {
    ...
    if (activeTab === 'planned') {
      ...
      // Обычный платеж
      transaction = QuickPaymentService.createQuickPayment(form, context);
    }
      ...
      <div className="bg-white rounded-xl shadow-xl max-w-3xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex justify-between items-center p-6 border-b border-slate-200">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-xl font-bold text-slate-800">Управление платежами</h3>
              <p className="text-sm text-slate-500">Создание и планирование платежей</p>
            </div>
          </div>
        ...
        <div className="flex border-b border-slate-200">
          {[
            { id: 'income' as PaymentTab, label: 'Поступление', icon: TrendingUp, color: 'green' },
            { id: 'expense' as PaymentTab, label: 'Расход', icon: TrendingDown, color: 'red' },
            { id: 'planned' as PaymentTab, label: 'Планирование', icon: Target, color: 'purple' }
          ].map(tab => (
            <button
              key={tab.id}
              onClick={() => setActiveTab(tab.id)}
              className={`flex items-center gap-2 px-4 py-3 border-b-2 transition-colors flex-1 ${
                activeTab === tab.id
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-xs font-bold text-slate-500 uppercase mb-1">
                {activeTab === 'planned' ? 'Планируемая дата' : 'Дата'}
              </label>
              <div className="relative">
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                placeholder={activeTab === 'planned' ? "Например: Планируемый платеж поставщику" : "Например: Оплата по договору"}
              />
            </div>
          </div>
          ...
          {/* Исполнение из графика смет (только для поступлений) */}
          {activeTab === 'income' && form.project_id && (
        ...
        <div className="flex justify-between p-6 border-t border-slate-200">
          ...
          <div className="flex gap-2">
            ...
            >
              {activeTab === 'planned' ? 'Запланировать' : 'Создать платеж'}
            </button>
          </div>
        </div>
      </div>
