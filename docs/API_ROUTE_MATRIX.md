...
## Контрактный smoke (Phase A / A4)

Скрипт: `scripts/smoke_api_contract.sh`

Примеры запуска:
- Только health checks:
  - `BASE_URL=http://localhost:8080 ./scripts/smoke_api_contract.sh`
- Полный smoke с авторизацией:
  - `BASE_URL=http://localhost:8080 EMAIL=user@example.com PASSWORD=your_password ./scripts/smoke_api_contract.sh`
