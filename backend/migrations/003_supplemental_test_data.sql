...
BEGIN
    ...
    RAISE NOTICE 'Контрагенты найдены: клиент=%, подрядчик=%', client_counterparty_uuid, contractor_counterparty_uuid;
    ...
    RAISE NOTICE 'Проект найден: %', project_uuid;
    ...
    IF NOT EXISTS (SELECT 1 FROM project_teams WHERE project_id = project_uuid AND user_id = manager_user_uuid) THEN
        ...
        VALUES (project_uuid, manager_user_uuid, 'Проектный менеджер');
    END IF;

    IF NOT EXISTS (SELECT 1 FROM project_teams WHERE project_id = project_uuid AND user_id = admin_user_uuid) THEN
        INSERT INTO project_teams (project_id, user_id, role)
        VALUES (project_uuid, admin_user_uuid, 'Администратор');
    END IF;
    ...
    -- Для земляных работ
    IF NOT EXISTS (SELECT 1 FROM price_items WHERE name = 'Разработка грунта вручную') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM price_items WHERE name = 'Разработка грунта механизированная') THEN
        INSERT INTO price_items (category_id, name, unit, cost_price, markup) 
        SELECT 
            pc.id,
    ...
    IF NOT EXISTS (SELECT 1 FROM price_items WHERE name = 'Засыпка и уплотнение') THEN
        INSERT INTO price_items (category_id, name, unit, cost_price, markup) 
        SELECT 
            pc.id,
    ...
    -- Для бетонных работ
    IF NOT EXISTS (SELECT 1 FROM price_items WHERE name = 'Устройство бетонной подготовки') THEN
        INSERT INTO price_items (category_id, name, unit, cost_price, markup) 
        SELECT 
            pc.id,
    ...
    IF NOT EXISTS (SELECT 1 FROM price_items WHERE name = 'Монолитная железобетонная плита') THEN
        INSERT INTO price_items (category_id, name, unit, cost_price, markup) 
        SELECT 
            pc.id,
    ...
    IF NOT EXISTS (SELECT 1 FROM price_items WHERE name = 'Монолитные стены') THEN
        INSERT INTO price_items (category_id, name, unit, cost_price, markup) 
        SELECT 
            pc.id,
            'Монолитные стены',
    ...
    IF NOT EXISTS (SELECT 1 FROM price_items WHERE name = 'Монолитные колонны') THEN
        INSERT INTO price_items (category_id, name, unit, cost_price, markup) 
        SELECT 
            pc.id,
            'Монолитные колонны',
    ...
    IF NOT EXISTS (SELECT 1 FROM price_items WHERE name = 'Кирпичная кладка наружных стен') THEN
        INSERT INTO price_items (category_id, name, unit, cost_price, markup) 
        SELECT 
            pc.id,
    ...
    IF NOT EXISTS (SELECT 1 FROM price_items WHERE name = 'Кирпичная кладка внутренних стен') THEN
        INSERT INTO price_items (category_id, name, unit, cost_price, markup) 
        SELECT 
            pc.id,
    ...
    IF NOT EXISTS (SELECT 1 FROM price_items WHERE name = 'Теплоизоляция стен') THEN
        INSERT INTO price_items (category_id, name, unit, cost_price, markup) 
        SELECT 
            pc.id,
            'Теплоизоляция стен',
    ...
    IF NOT EXISTS (SELECT 1 FROM price_items WHERE name = 'Штукатурка стен') THEN
        INSERT INTO price_items (category_id, name, unit, cost_price, markup) 
        SELECT 
            pc.id,
            'Штукатурка стен',
    ...
    IF NOT EXISTS (SELECT 1 FROM project_events WHERE project_id = project_uuid AND title = 'Начало строительства') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM project_events WHERE project_id = project_uuid AND title = 'Переход к основному этапу') THEN
        INSERT INTO project_events (project_id, user_id, type, title, description) VALUES
    ...
    IF NOT EXISTS (SELECT 1 FROM project_events WHERE project_id = project_uuid AND title = 'Получен авансовый платеж') THEN
        INSERT INTO project_events (project_id, user_id, type, title, description) VALUES
